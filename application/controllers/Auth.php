...
class Auth extends CI_Controller
{
    ...
    public function  registration()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            Berhasil membuat akun. Harap aktivasi akun anda</div>');
            redirect('auth');
        }
    }
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'webcapil8000@gmail.com',
            'smtp_pass' => 'webcapil8@',
            'smtp_port' => 465,
            'mailtype'  => 'html',
            'charset'   => 'utf-8',
        ...
        $this->email->from('webcapil8000@gmail.com', 'Web sistem-capil');
        $this->email->to($this->input->post('email'));

        if ($type == 'verify') {
            $this->email->subject('Verifikasi Akun');
            $this->email->message('Klik link ini untuk verifikasi akun anda : <a href="' .
                base_url() . 'auth/verify?email=' . $this->input->post('email') . '&token=' .
                urlencode($token) . '">Aktivasi</a>');
        } else if ($type == 'forgot') {
            $this->email->subject('Reset Password');
            $this->email->message('Klik link ini untuk mereset password anda : <a href="' .
        ...
        if ($user) {
            ...
            if ($user_token) {
                if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
                    ...
                    $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
                    ' . $email . ' telah diaktivasi! Harap Login.</div>');
                    redirect('auth');
                } else {
    ...
    public function forgotPassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($user) {
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
                Harap cek email anda untuk reset password!</div>');
                redirect('auth');
            } else {
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
    ...
    public function changePassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
            Password telah diganti! Harap login.</div>');
            redirect('auth/forgotpassword');
        }
    }
}
